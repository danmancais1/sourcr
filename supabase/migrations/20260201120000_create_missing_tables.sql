-- Create all app tables if missing. Safe to run on empty DB or when some tables exist.
-- Tables and schema match usage in codebase; RLS uses auth.uid() and workspace_members.

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Helper: user's workspace IDs (required for RLS)
...
-- RLS policies: drop if exists then create (idempotent)
...
CREATE POLICY "Members can read workspace" ON public.workspaces FOR SELECT USING (id IN (SELECT public.user_workspace_ids(auth.uid())));
CREATE POLICY "Members can update workspace" ON public.workspaces FOR UPDATE USING (id IN (SELECT public.user_workspace_ids(auth.uid())));
...
DROP POLICY IF EXISTS "Authenticated can add self to new workspace" ON public.workspace_members;
CREATE POLICY "Members can read workspace_members" ON public.workspace_members FOR SELECT USING (workspace_id IN (SELECT public.user_workspace_ids(auth.uid())));
CREATE POLICY "Members can insert workspace_members" ON public.workspace_members FOR INSERT WITH CHECK (workspace_id IN (SELECT public.user_workspace_ids(auth.uid())));
CREATE POLICY "Authenticated can add self to new workspace" ON public.workspace_members FOR INSERT WITH CHECK (
  user_id = auth.uid() AND NOT EXISTS (SELECT 1 FROM public.workspace_members w WHERE w.workspace_id = workspace_id)
);

DROP POLICY IF EXISTS "Members can manage api_keys" ON public.workspace_api_keys;
CREATE POLICY "Members can manage api_keys" ON public.workspace_api_keys FOR ALL USING (workspace_id IN (SELECT public.user_workspace_ids(auth.uid())));

DROP POLICY IF EXISTS "Members can manage owners" ON public.owners;
...
DROP POLICY IF EXISTS "Members can manage signals" ON public.signals;
...
DROP POLICY IF EXISTS "Members can manage campaign_steps" ON public.campaign_steps;
...
CREATE POLICY "Members can manage signals" ON public.signals FOR ALL USING (lead_id IN (SELECT id FROM public.leads WHERE workspace_id IN (SELECT public.user_workspace_ids(auth.uid()))));
...
CREATE POLICY "Members can manage campaign_steps" ON public.campaign_steps FOR ALL USING (campaign_id IN (SELECT id FROM public.campaigns WHERE workspace_id IN (SELECT public.user_workspace_ids(auth.uid()))));

DROP POLICY IF EXISTS "Members can read outbox" ON public.outbox_messages;
DROP POLICY IF EXISTS "Members can insert outbox" ON public.outbox_messages;
DROP POLICY IF EXISTS "Members can manage suppression_list" ON public.suppression_list;
...
DROP POLICY IF EXISTS "Anyone can create landlord_submissions" ON public.landlord_submissions;
DROP POLICY IF EXISTS "Members can read matched submissions" ON public.landlord_submissions;
DROP POLICY IF EXISTS "Sellers can read own submissions by email" ON public.landlord_submissions;
CREATE POLICY "Anyone can create landlord_submissions" ON public.landlord_submissions FOR INSERT WITH CHECK (true);
CREATE POLICY "Members can read matched submissions" ON public.landlord_submissions FOR SELECT USING (id IN (SELECT landlord_submission_id FROM public.matches WHERE workspace_id IN (SELECT public.user_workspace_ids(auth.uid()))));
CREATE POLICY "Sellers can read own submissions by email" ON public.landlord_submissions FOR SELECT USING (contact_email = (auth.jwt() ->> 'email'));

DROP POLICY IF EXISTS "Members can manage investor_buy_box" ON public.investor_buy_box;
DROP POLICY IF EXISTS "Members can manage matches" ON public.matches;
DROP POLICY IF EXISTS "Sellers can read own matches" ON public.matches;
...
CREATE POLICY "Sellers can read own matches" ON public.matches FOR SELECT USING (landlord_submission_id IN (SELECT id FROM public.landlord_submissions WHERE contact_email = (auth.jwt() ->> 'email')));
CREATE POLICY "Members can manage messages" ON public.messages FOR ALL USING (match_id IN (SELECT id FROM public.matches WHERE workspace_id IN (SELECT public.user_workspace_ids(auth.uid()))));

DROP POLICY IF EXISTS "Authenticated can manage cache_http" ON public.cache_http;
DROP POLICY IF EXISTS "Authenticated can read ppd" ON public.ppd_transactions;
DROP POLICY IF EXISTS "Authenticated can insert ppd" ON public.ppd_transactions;
DROP POLICY IF EXISTS "Members can read daily_send_count" ON public.daily_send_count;
DROP POLICY IF EXISTS "Members can update daily_send_count" ON public.daily_send_count;
